...
\begin{abstract}
  ...
  a unique window into the \ilya{\ac{BBH}} formation environment.  \sout{of massive stars and
  ...
  ($\OOneOddsIsoAligned$ odds ratio\footnote{An odds ratio of $r$ with
    ...
    normal CDF. \ilya{[Two-tailed vs.~one-tailed]}})  preference for an isotropic angular distribution.
...
\begin{equation}
  ...
  \right) \cdot \frac{\vec{L}}{\left| \vec{L} \right|} \ilya{\equiv} \frac{1}{M} \left( m_1 \chi_1 + m_2 \chi_2 \right),
\end{equation}
where \sout{the $\hat{z}$ axis is aligned with the orbital angular momentum
of the binary,} $m_{1,2}$ are the gravitational masses of the more-massive (1) and
less-massive (2) components, $M = m_1 + m_2$ is the total mass,
$\vec{S}_{1,2}$ are the spin angular momentum vectors of the black
holes in the binary, $\vec{L}$ is the orbital angular momentum vector \ilya{assumed to point in the $\hat{z}$ direction},
...
additional \sout{10} detections with similar observational uncertainties
