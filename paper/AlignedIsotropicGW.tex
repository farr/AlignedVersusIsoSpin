...
parameter space that describes the compact binary source.  The model
waveforms incorporate approximations to the true behaviour of the
merging systems that are expected to break down for sufficiently high
mis-aligned spins; detailed comparisons with numerical relativity
computations using no approximations to the dynamics
\citep{2016PhRvD..94f4035A} show no evidence of such a breakdown in
GW150914's analysis.  Given the preference of our data for smaller
spins (discussed below) we assume here that measurements made now and
in the future are not dominated by systematic errors.
...
aligned models; thus the aligned/isotropic discrimitating power in
...
We thank Richard O’Shaughnessy for comments on this work.  
