...
\begin{abstract}
  The first direct detections of \acp{GW}
  \cite{2016PhRvL.116f1102A,2016PhRvL.116x1103A,O1-BBH} from merging
  \acp{BBH} open a unique window into the \ac{BBH} formation
  environment.  One promising signature of the formation environment
  is the angular distribution of the \ac{BH} spins; systems formed
  through dynamical interactions among already-compact objects are
  expected to have isotropic spin orientations
  \cite{SigurdssonHernquist:1993,1993Natur.364..421K,PZMcMillan:2000,Rodriguez:2015,Stone:2016,2016ApJ...832L...2R}
  ...
  a consequence of their joint evolution toward a \ac{BBH} system
  \cite{TutukovYungelson:1973,TutukovYungelson:1993,Lipunov:1997,2016Natur.534..512B,Stevenson:2017,MandeldeMink:2016,Marchant:2016}.
  We consider existing \ac{GW} measurements of $\chieff$, the
  best-measured combination of spin parameters
  \cite{2016PhRvL.116x1102A,O1-BBH}, in the three likely binary black
  hole detections GW150914, LVT151012, and GW151226.  If binary black
  hole spin magnitudes extend to high values, as is suggested by
  observations of black hole X-ray binaries
  \cite{2015PhR...548....1M}, we show that the data already exhibit a
  $\OOneSigmaIsoAligned\sigma$ ($\OOneOddsIsoAligned$ odds
  ratio\footnote{An odds ratio of $r$ with $r \ll 1$ is equivalent to
    $x \sigma$ with $x = \Phi^{-1}\left( 1 - r/2 \right)$, where
    $\Phi$ is the unit normal CDF.})  preference for an isotropic
  angular distribution.  By considering the effect of 10 additional
  detections \cite{2016ApJ...833L...1A} drawn from the various models
...
\cite{2015PhRvD..91d2003V} compare model gravitational waveforms
\cite{2014PhRvD..89h4006P,2014PhRvD..89f1502T,2014PhRvL.113o1101H}
...
credible interval as quoted in \cite{O1-BBH}.  None of the $\chieff$
posteriors are consistent with two black holes with large aligned
spins, $\chi_{1,2} \gtrsim 0.5$; this contrasts with the large spins
inferred for the majority of black holes in X-ray binaries with claimed spin measurements (see
...
\cite{2016PhRvL.116x1103A,O1-BBH}.
...
\begin{figure}
  ...
  \caption{\label{fig:O1-posteriors} Approximate posteriors on
    ...
    \cite{O1-BBH}.  We approximate the posteriors reported in
    \cite{O1-BBH} using Gaussians with the same median and 90\%
    credible interval as reported in \cite{O1-BBH}.  It is notable
    that none of the $\chieff$ posteriors support high \ac{BH}
    spin magnitudes with aligned spins, suggested by observations of stellar-mass black holes in  X-ray binaries (see
    \cite{2015PhR...548....1M} for a summary of such measurements).}
\end{figure}
...
consistent with the three observations \cite{O1-BBH}, and the
...
\cite{O1-BBH,2016ApJ...833L...1A,2016ApJS..227...14A}.  Assuming 10
additional detections drawn from each of our six zero-parameter models
for the spin distribution in addition to the three existing detections
from O1, with observational uncertainties drawn randomly from the
three Gaussian widths used to approximate the $\chieff$ posteriors in
Figure \ref{fig:O1-posteriors}\footnote{The measurement uncertainty in
  $\chieff$ depends on the other parameters of the merging \ac{BBH}
  ...
  detections. See Section \ref{sec:chi-eff-precision}
for further discussion.}, we find the odds ratios shown in Figure
...
the GW151226 observations \cite{2016PhRvL.116x1103A}.
...
\cite{2015PhR...548....1M} give current estimates of the spin
...
complicate either type of analysis (see \cite{2015PhR...548....1M} for
a discussion), and selection effects could yield a biased
...
presented by \cite{1999MNRAS.305..654K}), but see
\cite{2003MNRAS.341..385P,2015ApJ...800...17F}.  Thus the current spin
parameters probably are close to their values upon core collapse, at
least in high-mass X-ray binaries.  However, the specific processes
...
\cite{2016MNRAS.462..844K,2017arXiv170200885Z,2017arXiv170203952H}.

The spin directions of binary black holes formed dynamically through
interactions in dense stellar environments
\cite{SigurdssonHernquist:1993,1993Natur.364..421K,PZMcMillan:2000,Rodriguez:2015,Stone:2016}
are expected to be isotropic given the absence of a preferred
direction \cite{2016ApJ...832L...2R} and the persistence of an
isotropic distribution through post-Newtonian evolution
\cite{2004PhRvD..70l4020S,2007ApJ...661L.147B}.

The spin directions in isolated binaries, whether evolving via the
classical channel through a common-envelope phase
\cite{TutukovYungelson:1973,TutukovYungelson:1993,Lipunov:1997,2016Natur.534..512B,Stevenson:2017}
or through chemically homogeneous evolution
\cite{MandeldeMink:2016,Marchant:2016} are usually expected to be
preferentially aligned.  Despite observed spin-orbit misalignments in
both massive stellar binaries
\cite{Albrecht:2009,2014ApJ...785...83A} and BH X-ray binaries
\cite{Orosz:2001,Martin:2008b,Martin:2008,MorningstarMiller:2014},
...
\cite{2000ApJ...541..319K,2013PhRvD..87j4028G}; the supernova can
also tilt the spin angle, as in the double pulsar J0737-3039
\cite{2011ApJ...742...81F}; and a variety of uncertain processes,
...
the binary spirals in through \ac{GW} emission \cite{2015PhRvD..92f4016G},
but $\chieff$ is approximately conserved through this evolution.

\cite{2017CQGra..34cLT01V} also studied the possibility of
...
in agreement with the present study, \cite{Stevenson:2017spin} found
...
\cite{2017arXiv170306869F} and \cite{2017arXiv170306223G} focused on inference
on spin magnitudes.
...
\begin{addendum}
\item We thank Richard O'Shaughnessy, Christopher Berry, and Davide
  Gerosa for discussions and comments on this work.  WF, SS, IM and AV
  were supported in part by the STFC.  MCM acknowledges support of the
...
\caption{Fraction of the BBH population coming from an isotropic
...
Following \cite{Stevenson:2017spin}, we fit a mixture model (labelled
...
\caption{Effect of small spins on evidence ratio of aligned against
  isotropic models. The blue line shows the evidence ratio (plotted as
  the equivalent sigma) between a model where all systems are aligned,
  versus one where all systems are from an isotropic distribution as a
  function of the power law $\alpha$ corresponding to
  Equation~\ref{eq:lowspinalpha}.  The top axis shows the mean spin
...
\cite{2016arXiv161107531T}.  For this source, statistical uncertainties dominate
...
\cite{2016PhRvD..94f4035A} also suggest that statistical
uncertainties dominate the systematics for this system.
\cite{2016arXiv161107531T} suggests that systematics may dominate for
...
detector to select only coincident events.} of 500 binaries was selected
...
locations distributed uniformly in volume, and orientations distributed
isotropically.  Data were simulated for each binary, and posteriors were
estimated using the LIGO-Virgo parameter estimation library
...
\begin{figure}
  ...
  \caption{Widths of the 90\% credible intervals for $\chieff$ for 500
    binaries in a simulated detected population. $\chieff$ is better
    constrained for systems with low $\chieff$ and low mass ratio.}
