...
\begin{abstract}
  ...
  a unique window into the \ac{BBH} formation environment.  One
  promising signature of the formation environment is the angular
  distribution of the \ac{BH} spins; systems formed through dynamical
  interactions among already-compact objects are expected to have
  isotropic spin orientations whereas binaries formed from pairs of
  stars born together are more likely to have spins preferentially
  aligned with the binary orbit as a consequence of their joint
  evolution toward a \ac{BBH} system.  We consider existing \ac{GW}
  measurements of $\chieff$, the best-measured combination of spin
  parameters, in the three likely binary black hole detections
  GW150914, LVT151012, and GW151226.  If binary black hole spin
  magnitudes extend to high values, as is suggested by observations of
  black hole X-ray binaries, we show that the data already exhibit a
  ...
  detections drawn from the various models in the suite we show that
  if all observations come from a single population such an augmented
  data set would enable at least a $\OTwoSigmaIsoAlignedMin\sigma$
